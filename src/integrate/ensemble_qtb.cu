  ...
  std::vector<double>& corrected_theta)
{
  ...
  if (N_f >= 4 && domega > 0.0 && fric_coef > 0.0) {
    // The theta-tilde deconvolution becomes ill-conditioned in the first few
    // bins near omega = 0, where the exact solution must smoothly recover the
    // classical limit. Blend those bins back to a stable low-frequency anchor
    // over approximately one Langevin linewidth.
  ...
  const std::vector<double>& adaptive_rate_type_input)
{
  ...
  if (adaptive_window_input > 0.0) {
    int segment_estimate = int(floor(adaptive_window_input / alpha + 0.5));
...
void Ensemble_QTB::initialize_adaptive_qtb()
{
  ...
  adaptive_gamma_file = my_fopen("qtb_adaptive_gamma.out", "w");
  adaptive_fdt_file = my_fopen("qtb_adaptive_fdt.out", "w");
  adaptive_theta_file = my_fopen("qtb_theta_correction.out", "w");
  fprintf(
    adaptive_gamma_file,
...
{
  const double time_ps = (*current_step) * dt * TIME_UNIT_CONVERSION / 1000.0;
...
void Ensemble_QTB::update_time_filter(const double target_temperature)
{
  ...
  for (int filter = 0; filter < time_filter_count; ++filter) {
    ...
    for (int k = 0; k < nfreq2; ++k) {
      ...
      const double omega_k = positive_index * PI / (N_f * h_timestep);
      const double g_k = get_ou_spectrum_correction(omega_k, fric_coef, h_timestep);
...
void Ensemble_QTB::adapt_random_force_spectrum()
{
  ...
  for (int type = 0; type < number_of_atom_types; ++type) {
    ...
    const double atom_mass = atom_type_masses_host[type];
    const double half_decay = exp(-0.5 * fric_coef * h_timestep);
    const double half_response =
      fric_coef > 0.0 ? (1.0 - half_decay) / fric_coef : 0.5 * h_timestep;
    ...
    for (int k = 0; k <= N_f; ++k) {
      const int gamma_index = type_offset + get_gamma_index_from_fft_bin(N_f, k);
      const double gamma_k = gamma_spectrum_host[gamma_index];
      const double adaptive_bin = double(k) * adaptive_segment_length / nfreq2;
      ...
      // The random force is sampled as a blockwise-constant quantity on the
      // coarse h_timestep grid, while the velocity stored in the history is
      // the end-of-block BAOAB velocity. Recover the velocity/random-force
      // correlation at the block center by inverting the exact half-block
      // Ornstein-Uhlenbeck propagation.
      adaptive_vr_host[type_offset + k] =
        (adaptive_vr_raw_host[type_offset + k] -
         half_response * adaptive_ff_host[type_offset + k] / atom_mass) /
        half_decay;
  ...
  GPU_Vector<double>& thermo)
{
  ...
  if (counter_mu == alpha - 1) {
    // The colored random force is refreshed once per h_timestep = alpha * dt
    // block and then kept constant within that block. Sample the velocity only
    // after the full block has elapsed so that the coarse-grained velocity and
    // random-force histories are defined on the same time grid.
    sample_adaptive_qtb();
  }
